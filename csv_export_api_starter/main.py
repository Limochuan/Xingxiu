...
app = FastAPI(title="Yaoguang Excel Download API", version="1.0.0")
...
def parse_date_range(raw: str) -> Tuple[Optional[dt.date], Optional[dt.date]]:
    ...
    - "2025/10/01—2025/10/15"
    - 以及用户示例里的“2025--10-15”异常连字符，尽量宽松处理
    """
    if not raw:
        return None, None
    # 统一分隔符为 -
    s = raw.strip()
    s = s.replace("—", "-").replace("–", "-").replace("to", "-").replace("TO", "-").replace("到", "-")
    s = re.sub(r"-{2,}", "-", s)  # 多个连字符压缩
    ...
    if len(parts) >= 2:
        left = parts[0]
        right = parts[-1]
        d1 = parse_date_like(left)
        d2 = parse_date_like(right)
        return d1, d2
    return None, None

def detect_date_span_from_rows(rows: List[Dict[str, Any]]) -> Tuple[Optional[dt.date], Optional[dt.date]]:
    """
    如果用户没传日期，就从结果里推断 DATE_STR 的最小/最大，前提是有 DATE_STR 字段（字符串或日期）
    """
    if not rows:
        return None, None
    dates = []
...
) -> Tuple[str, List[Any], Optional[Tuple[dt.date, dt.date]], bool]:
    """
    返回 (where_sql, params, (span_start, span_end), single_day_flag)
    """
    where = []
    params: List[Any] = []
...
def col_widths_spec() -> List[float]:
    # A..AE 共 31 列
    # 索引 0→A，…，25→Z，26→AA，27→AB，28→AC，29→AD，30→AE
    widths = [
...
def make_excel(
    ...
    # 如果数据为空，用 ordered_cols 填表头；否则用 rows[0] 的键顺序与 ordered_cols 对齐
    headers = ordered_cols

    for col_idx, col_name in enumerate(headers, start=1):
        c = ws.cell(row=2, column=col_idx)
        # 每个单元格两行字：这里用“字段名 + 空行”实现两行换行
        c.value = f"{col_name}\n"
        c.font = header_font
...
def download_excel(
    ...
    # 查询（若完全无过滤，则默认用 PROJECT_NAME=默认项目）
    if not where_sql:
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            # 仅保留前 31 列，缺失列以空补齐（写入时按 ordered_cols 名取值）
    ...
    if span[0] and span[1]:
        if single_day:
            date_tag = span[0].strftime("%Y%m%d")
        else:
            date_tag = f"{span[0].strftime('%Y%m%d')}-{span[1].strftime('%Y%m%d')}"
    else:
